...
int main(int argc, char *argv[])
{
	...
	for (;;){
		...
		while (terminar ==0)
		{
			...
			sprintf(consulta2,"SELECT Jugador.nombre FROM Partida,Historial,Jugador WHERE Partida.ID=%d AND Partida.ID=Historial.partida AND Historial.jugador=Jugador.ID AND Jugador.sexo='hombre'" ,partidaID);
			...
			while(row!=NULL){
			sprintf(respuesta,"Nombre de jugador en la partida %d que son hombres :%s \n",partidaid,row[0]);
			row=mysql_fetch_row(resultado);
			strcpy (respuesta,"NO");
